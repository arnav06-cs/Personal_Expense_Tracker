...
def generate_report():
    ...
    def add_image(img_path, caption_text=""):
    ...
    add_h2("1.7 User Journey & Persona Scenarios")
    add_p("NovaSpend was designed around three distinct user personas to optimize real-world user experience:")
    add_bullet("College student managing a tight monthly allowance of ₹10,000. Needs real-time budget utilization alerts and gamified savings motivation to prevent overspending in the first two weeks of the month.", "Persona A (Undergraduate Student):")
    add_bullet("Software engineer earning variable monthly project payouts. Requires itemized income logging, custom category expense tracking (Cloud, Subscriptions, Tools), and instant PDF report generation for client tax billing.", "Persona B (Freelance Engineer):")
    add_bullet("Shared accommodation roommates incurring collective grocery and dining expenses. Requires the Group Bill Splitter tool to instantly calculate individual per-person shares and log them directly into personal expense categories.", "Persona C (Group Roommates):")

    add_h2("1.8 Report Organization Structure")
    add_p("This Vocational Training Report is organized into five comprehensive technical chapters:")
    add_bullet("Details organizational background, problem statement, core project objectives, scope, technology stack rationale, persona scenarios, and report organization.", "Chapter I (Introduction):")
    add_bullet("Defines complete software and hardware requirement specifications, feasibility studies, environment configurations, and security protocols.", "Chapter II (Software & Hardware Requirements):")
    ...
    add_bullet("Provides an exhaustive walkthrough of all 11 project modules, source code implementations, test case matrices, and performance metrics.", "Chapter IV (Results & Discussions):")
    ...
    add_h2("2.4 Risk Assessment & Security Protocols")
    add_p("Potential technical risks and their mitigation protocols were systematically cataloged:")
    add_table_data(
        ["Risk Dimension", "Potential Failure Mode", "Severity / Impact", "Mitigation Strategy"],
        [
            ["Database Loss", "Accidental sqlite3 file corruption", "HIGH", "Automated daily SQLite backup script to isolated backup storage."],
            ["Unauthorized Access", "Session hijacking / XSS attack", "HIGH", "Set HTTPOnly & SameSite flags on session cookies; escape HTML input."],
            ["Data Breach", "Horizontal data access across users", "CRITICAL", "Enforce request.user filtering on all ORM queries; @login_required."],
            ["File Upload Exploit", "Malicious file upload via receipt field", "MEDIUM", "Validate image extensions via Pillow (PIL); restrict media permissions."],
            ["Overspending Risk", "User unaware of budget depletion", "MEDIUM", "Real-time utilization progress bar with red warning triggers (>90%)."]
        ]
    )

    add_h2("2.5 Development & Environment Configuration")
    ...
    add_image(
        r'C:\Users\aaap7\Documents\Personal_Expense_Tracker\scratch\system_flowchart.png',
        "Figure 3.1: NovaSpend System Architecture & End-to-End Data Execution Flowchart"
    )

    add_h2("3.3 URL Pattern & Dispatcher Mapping")
    add_p("The web application maps incoming HTTP request routes to view handlers in `expenses/urls.py` as detailed in the routing table below:")
    add_table_data(
        ["URL Route Pattern", "HTTP Method", "View Function Handler", "Functional Description"],
        [
            ["/", "GET", "expenses.views.dashboard", "Renders Executive Dashboard with KPI cards & doughnut chart."],
            ["/login/", "GET / POST", "django.contrib.auth.views.LoginView", "Authenticates user & initiates session."],
            ["/logout/", "POST", "django.contrib.auth.views.LogoutView", "Terminates active user session."],
            ["/register/", "GET / POST", "expenses.views.register", "Registers new user account & initializes UserProfile."],
            ["/expenses/", "GET", "expenses.views.expense_list", "Lists itemized expenses with search & filter controls."],
            ["/expenses/add/", "GET / POST", "expenses.views.expense_create", "Creates new expense entry with receipt upload."],
            ["/expenses/<id>/edit/", "GET / POST", "expenses.views.expense_update", "Updates existing expense record."],
            ["/expenses/<id>/delete/", "POST", "expenses.views.expense_delete", "Deletes expense record."],
            ["/income/", "GET / POST", "expenses.views.income_list", "Logs & displays itemized income sources."],
            ["/categories/", "GET / POST", "expenses.views.category_list", "Manages custom categories & hex color pickers."],
            ["/budget/", "GET / POST", "expenses.views.budget", "Configures monthly budget cap & utilization meter."],
            ["/ai-advisor/", "GET", "expenses.views.ai_advisor", "Renders NovaAI Financial Advisor chat interface."],
            ["/arcade/", "GET", "expenses.views.arcade_view", "Renders Savings Arcade, XP bar, 10 badges & rank modal."],
            ["/change-currency/", "POST", "expenses.views.change_currency", "Updates user profile active currency symbol."],
            ["/split-bill/", "GET", "expenses.views.split_bill", "Renders Group Bill Splitter calculation engine."],
            ["/reports/pdf/", "GET", "expenses.views.export_pdf", "Generates & downloads ReportLab PDF ledger report."],
            ["/reports/excel/", "GET", "expenses.views.export_excel", "Generates & downloads OpenPyXL .xlsx spreadsheet."]
        ]
    )

    add_h2("3.4 Relational Database Schema Specifications")
    add_p("The database consists of six core relational tables designed with referential integrity:")

    add_image(
        r'C:\Users\aaap7\Documents\Personal_Expense_Tracker\scratch\er_diagram.png',
        "Figure 3.2: NovaSpend Database Entity-Relationship (E-R) Diagram & Relational Cardinalities"
    )
    ...
    add_h2("3.5 Core Model Architecture Source Declarations")
    ...
    add_h2("3.6 System Security & Data Isolation Architecture")
    ...
    add_h2("4.2 Core View Handler Implementations")
